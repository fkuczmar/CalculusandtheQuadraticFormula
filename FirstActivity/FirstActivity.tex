...
It's been more than a quarter-century now, but the conversation I had with Stephanie opening-week of our first year at Shoreline still resonates in my mind.

Stephanie made the point that when you solve a problem with numbers, there are only two questions left. Is your answer reasonable? And what is the next question?

But when you solve a problem in general, you can ask first if the answer has the correct units (or if it is dimensionallly correct). And then you can ask more sophisticated questions about what happens when you vary one or more of the parameters.

While the latter question hints at partial derivatives, it still seems like it might be used effectively earlier in our courses. But even at the calculus level, partial derivatives could be introduced earlier. In fact, MaCalester College starts their calculus sequence with multivariable calculus and claims to have had much success in doing so.

I have found it worthwhile to focus on the first question. At first, students don't understand what it means to check units, but they quickly catch on. And it seems like a good idea to start students working in general early on.

The ideas in this note started with a question about units that asks to check the quadratic formula is dimensionally correct.

I suppose the answer to this question depends on the application, but we'll start with the most basic scenario where the parabola
\[
  y  = ax^2 + bx + c \, , \, a\neq 0 ,
\]
is just a curve in the plane and the coordinates are measured in meters.

Now we can interpret the quadratic formula as giving directions to locate the parabola's $x$-intercepts. How we might interpret these directions depends on our perspective. We'll start with Math 151..

\begin{enumerate}
\item The first step is to think about the coefficients $c$, $b$, $a$. What are their units?

\item The next step is to interpret the coefficients geometrically.  %Give geometric interpretations of $c$, $b$, $a$.

\item Now let's split the quadratic formula into two terms and say the solutions to the equation 
\[
     0 = ax^2 + bx + c \, , \, a\neq 0
\]
are
...
\item What are the units of $-b/2a$? 

\item The key now is to interpret the meaning of this first term geometrically. Here's a little hint. Think of this term as a change
\[
  \Delta x = - \frac{b}{2a}
\]
in an $x$-coordinate. Then recall a math department meeting a few years ago when Trevor talked about two normal lines to a parabola.

In case you forgot, here's the problem.
...
Click on the blue arrow at the right to see a geometric interpretation of 
\[
 \Delta x =  - \frac{b}{2a} .
\]

\begin{expandable}
The normal line to the parabola at $(0,b)$ has slope $m_\perp = -1/b$. 

The first term records the change 
\[
        \Delta x =  \left( \frac{1}{m_\perp} \right) \Delta y = -b \left( \frac{1}{2a} \right)%\left( -\frac{1}{1/b} \right)
\]
in the $x$-coordindate moving along the normal line from the $y$-intercept to the axis of symmetry. 
...
\item To start on the second term, we should first think about its numerator,
\[
  \sqrt{b^2 - 4ac}.
\]
What the units here? And can you think of a geometric interpretation relevant to a 151 class?

\item Now what about the units of 
\[
...
From the pespective of a Math 151 student, the quadratic formula gives these directions to find the $x$-intercepts of the parabola $y=c+bx +ax^2$.

\begin{enumerate}
\item Start at the $y$-intercept $Q(0,c)$.


\item Then move along the normal at $Q$ until the $y$-coordinate changes by $1/(2a)$, thereby landing on the axis of symmetry. 

\item Now move vertically along the axis of symmetry to the point $R$ with $y$-coordinate $y=1/(2a)$.

\item Next draw all lines through $R$ normal to the parabola. There are two possibilities.

\begin{enumerate}

\item There is only one such line. If so, remain at $R$. If by chance $R$ also lies on the $x$-axis you have found the unique $x$-intercept of parabola. If not, stop your search. There are no $x$-intercepts.


\item There are two additional normal lines through $R$. If so, move along these until you hit the parabola. Then stop. You have reached the $x$-intercepts.
...
\begin{onlineOnly}
    \begin{center}
\desmos{j980gbvqm6}{450}{600} 
\end{center}
\end{onlineOnly}
\end{exploration}

\href{https://www.desmos.com/calculator/j980gbvqm6}{Calculus and the Quadratic Formula 23}


\section{Integration and the Quadratic Formula}


\begin{exploration}
\begin{onlineOnly}
    \begin{center}
\desmos{qpkh9mmjty}{450}{600} 
\end{center}
\end{onlineOnly}
\end{exploration}

\href{https://www.desmos.com/calculator/qpkh9mmjty}{Quadratic Formula and Integration}
