...
The figure below shows the ordinary cycloid traced by a point $P$ fixed on the circumference of a rolling wheel. The marked angle $\theta$ is the wheel's rotation angle measured from an arbitrary starting point, in this case from the vertical when $P$ coincides with the top point $A$ of the cycloid. Because of (b) above, the incination of the cycloid at $P$ (ie. the angle between the tangent and the horizontal) is equal to the measure of $\angle BQP$ (ie. the angle between the normal $\overline{PQ}$ and the vertical). The points on the cycloid are spaced at equal intervals of rotation and suggest the speed $v= \rho (d\theta/dt)$ of the tracing point is proportional to the rotation arm $PQ$. %its distance from the center of rotation.  
...
To prove the arclength theorem, it helps to think differentially. Then (a) is equivalent to saying that as the wheel turns through the differential angle $d\theta>0$, the tracing point traverses the differential distance $ds = \rho \, d\theta$. 
...
The conclusion seems suspect, but follows from the more general fact that the arithmetic mean (or more generally, the weighted average) of the velocities of two points at rest in the reference frame of the rolling wheel is equal to the velocity of the mean (or more generally the same weighted average) of these points.

Roll a book on the tops of two cyinders (oatmeal containers work well) and you can see the book advacne twice as far as the cylinders, a convincing way to demonstrate that at the top of its motion a pebble stuck in a tire moves twice as fast as the axle. 
