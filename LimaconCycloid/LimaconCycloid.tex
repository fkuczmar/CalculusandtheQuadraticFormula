...
Fix a point $P_1$ in the reference frame of a circle ${\cal C}_1$ of radius $a$. Now roll the circle once through its circumferene on a line ${\cal L}$ and let $P_1$ sweep out a curve. When $P_1$ is a distance $b>a$ from the circle's center the roulette crosses ${\cal L}$ and  is called a \emph{prolate} cylcoid (from the Latin \emph{poferre}, to extend). In a recent article,  D. Robinson made the intriguing observation that the length of the portion of the prolate cycloid on the same side of ${\cal L}$ as ${\cal C}_1$ exceeds the length on the opposite side by an amount equal to the length of the classic cycloid, the curve traced by a point $P_2$ fixed on the circumference of the same rolling circle (see Figure 1). 
...
Our first step in this direction is to look for a more general equality of arclengths by associating each point of the cycloid above the $x$-axis with a point below. It seems like the only choice was to pair points with same inclination angle (the angle a curve makes with the horizontal) and experiments with desmos suggest that the same equality of arclengths holds between any two inclination angles.
...
\begin{theorem9}
With $s_1$, $s_2$ the respective arclengths of the cycloid (\ref{Eq:Limacon}) above and below the $x$-axis between inclination angles $0$, and $\phi$ and $s_3$ the arclength of the classic cycloid (\ref{Eq:Cycloid}) between these same angles,
\[
s_1 - s_2 = s_3.
\]
\end{theorem9}
...
The same is true locally. As the rolling quarter turns the differential angle $d\theta$ about the center of the other, it rotates through the differential angle $d\theta^* = 2\, d\theta$ in the reference frame of the other. Consequently, the arclength of a cardioid is twice that of its related cycloid, both globally and locally. These curves are traced by points attached to a circle as it rolls once through its circumference; on a line for the cycloid and on a circle of the same radius for the cardioid.  

To see this, we need only attach tracing points $P$ (for the cycloid) and $P^*$ (for the cardioid) to the rolling quarters of Figure ??, taking care that in their initial positions they have the same positions relative to the respective points of tangency as in Figure ??.  Then as the circles roll through the arclength $d\sigma$ of their circumferences, these circles turn through the respective angles $d\theta = d\sigma/a$ and $P^*$ and $d\theta^* = 2\, d\theta$. Since the distances $\rho = QP$ and $\rho^* = Q^*P^*$ from the centers of rotation to the tracing points are equal, the point $P^*$ sweeps out an arclength
\[
      ds^* = QP\, d\theta^* = 2\,ds
\]
...
This equation also holds if ${\cal C}$ rolls on the inside of ${\cal C}_r$ if we suppose $r$ to be negative in this case.
