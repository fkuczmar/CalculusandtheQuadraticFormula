...
\title{Arclength Theorems for Cycloids and Epicycloids}

\newcommand{\pskip}{\vskip 0.1 in}

\newtheorem{theorem9}{Arclength Theorem for Cycloids}
\newtheorem{theorem10}{Arclength Theorem for Epicycloids}
...
In the figure below, for example, the difference $s_1 - s_2$ in the arclengths $A_1P_1$ and $A_2P_2$ in the cycloid between indclination angles $0$ and $\phi$ equals the arclength $s_3$ of the ordinary cycloid between these same angles. 
...
The same equation also holds if ${\cal C}$ rolls on the inside of ${\cal C}_r$ provided we take $r$ to be negative.

In either case, the equation implies a similar relationship 
\[
  ds^*  =  \left( 1 + \frac{a}{r}  \right)  ds
\]
between the differential arclengths $ds$ of a prolate cycloid and $ds^*$ of its sister epicycloid (or hypocycloid if $r<0$) between inclination angles $\phi$ and $\phi + d\phi$. The sister epicycloid is traced by a fixed point $b$ units from the center of a circle with radius $a$ rolling around the outside (or inside) of a circle of radius $r$. Here we suppose as for the cardioid, that we measure the inclination angle of the epicycloid relative to the rolling tangent.
...
An immediate consequence is an arclength theorem for epicycloids.

\begin{theorem10}
Let $s_1^*=s_1^*(\phi)$ and $s_2* = s_2^*(\phi)$ be the arclengths of an $(a,b,r)$ prolate epicycloid (or hypocycloid) between inclination angles $\phi$ and $\phi + d\phi$, the former on the same side of the tangent line as the rolling circle, the latter on the opposite side. 
...
\[
    ds_1 - ds_2 = ds_3
\]
for the respective arclengths of the $(a,b)$ prolate cycloid (above and below) and its sister cylcoid between inclination angles $\phi$ and $\phi + d\phi$. Multiplying both sides of the above equation by the factor $a/r + 1$ shows
\[
    ds_1^* - ds_2^* = ds_3^*
\]
...
Fgure 5 shows an example for the $(a,b,r)=(2,6,5)$ prolate epicyloid. Its red arclength exceeds the blue by the length of its sister $(a,r) = (2,6)$ epicycloid (dashed). The figure shows the rolling circle when the tracing point $P^*$ crosses the rolling tangent and the arclength of the prolate epicycloid changes sign.
...
\item \emph{Cardioids and Limacons:} When a circle rolls once around a fixed circle with the same radius (Figure 6), the $(a,b,a)$ prolate epicycloid is a limacon with polar equation $r=2a+2b\cos\theta$. Its arclength counts negatively around the loop of the limacon and the outer arclength exceeds the loop's length by the length $16a$ of the limacon's sister cardioid, twice that of the ordinary $a$-cycloid. 
