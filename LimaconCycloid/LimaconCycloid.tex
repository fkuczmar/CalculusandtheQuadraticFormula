...
The figure below makes the equality of arclengths plausible if you focus on the arcs with the same color at the same inclination angles.
...
It might seem counterintuitive that at any moment a rolling wheel is rotating not about its axle but rather about its point of contact with the ground. This statement has two implications for a point $P$ at rest in the reference frame of the rolling wheel, like a pebble stuck in a tire.
...
The figure below shows a cycloid, the curve traced by a fixed point $P$ on the circumference of a rolling wheel. The angle $\theta$ is the rotation angle of the wheel measured from an arbitrary starting point, in this case when $P$ coincides with $A$. Because of (b) above, the incination of the cycloid at $P$ (ie. the angle between the tangent and the horizontal) is equal to $\angle BQP$ (ie. the angle between the normal $\overline{PQ}$ and the vertical).

The points on the cycloid are spaced at equal intervals of rotation and suggest that the speed of the tracing is proportional to its distance from the center of rotation.  
...
\begin{proof}
In proving Theorem 1, it helps to imagine three circles rolling along the $x$-axis as the points $P_i$, $i=1,2,3$ at rest in the reference frames of these circles sweep out the upper limacon, lower limacon, and cycloid. The circles start with the points $P_i$ at the same inclination angle $\phi=0$ and the segments $Q_iP_i$ vertical. And as illustrated in Figure 3, the segments $\overline{Q_iP_i}$ from the centers of rotation to the tracing points remain parallel.
...
\[
   \rho_1 - \rho_2 = \rho_3 ,
\]
does \emph{not} implie the equality of differential arclengths (\ref{Eq:DiffArcs}) because the circles turn through unequal differential angles $d\theta_i$ in sweeping out the arcs. %We can see this globally since in sweeping out the full curves (upper limacon, lower limacon, and cycloid) the circles turn through the unequal angles
%\[
...
It turns out that we just need  two facts:
\begin{enumerate}
% \item $ds_i = \rho_i \, d\theta_i$ ,

\item $\theta_1 + \theta_2 = \theta_3$ (where the rotation angles are measured from the , and

\item $\frac{d\theta_1}{d\theta_2} = \frac{\rho_1}{\rho_2}$.
\end{enumerate}

These are enough to prove Theorem 1. For then
\begin{align*}
...
The proof of (a) follows from the composite figure below (note $\theta_2$ is measured from the downward vertical). The key point is that since $S_1P_1 = Q_1R_1 = \rho_2$, $\Delta C_1S_1P_1\cong \Delta C_1Q_1R_1$. Therefore, the marked angle $\angle P_1C_1S_1$ has measure $\theta_2$ and 
\[
 \theta_3 = \angle B_1C_1P_1 = \theta_1 + \theta_2.
\]
...
To prove (b), we could use the law of sines ...


\begin{onlineOnly}
    \begin{center}
\desmos{xvwr9mhuvw}{900}{600}   
\end{center}
\end{onlineOnly}

\href{https://www.desmos.com/calculator/xvwr9mhuvw}{Cylcoids Proof of Theorem 1B}
