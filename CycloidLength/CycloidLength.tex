...
So at the transition points, when rotation arm $\overline{Q_1P^*}$ is perpendicular to $\overline{P_1Q_1}$, rotation arm $\overline{Q_2P^*}$ is perpendicular to $\overline{P_2R_2}$ (from (a) and (b)) and hence (from (c)) $\overline{P_2R_2}$ is tangent to the ellipse. 

The proof of (a) and (b) rests on the fact that the radius of each rolling circle is half the radius of its partner. For then  
 (\ref{Eq:RollAngle}) implies the vertical lines (not shown) through centers $C_1$ and $C_2$ bisect the angles $\angle Q_1C_1P^*$ and $\angle Q_2C_2P^*$. It follows that $\overline{C_1Q_1}\parallel \overline{C_2P^*}$ (proving (a)) and $\overline{C_1P^*}\parallel \overline{C_2Q_2}$. So $\angle P^*C_1Q_1\cong \angle Q_2C_2P^*$ and $\Delta P^* C_1Q_1 \cong \Delta Q_2C_2P^*$ (SAS). Thus $\angle C_1P_1Q_1 \cong \angle C_2Q_2P^*$ and so $\overline{Q_2P^*}\parallel \overline{P^*Q_1}$ (proving (b)).
...
To prove (c), we let's focus our attention on this second trammel. With the coordinate system of Figure ?? and $\phi$ the marked angle between the arm and the $x$-axis, $P^*$ has coordinates 
\[
...
This shows $P^*$ sweeps out an ellipse with semi-major/minor axes $P^*R_2=b+a$ and $P^*P_2 = b-a$. From here we can take an algebraic or a geometric approach. In either case, the key observation the same - the sliding arm is tangent to the ellipse exactly when $P^*$ moves parallel to the arm. I like the algebraic approach for a calculus class (where this problem is a version of the problem about relating the speeds of the ends of a sliding ladder), but here we'll take a geometric approach by appealing once again to the instantaneous center of rotation.
