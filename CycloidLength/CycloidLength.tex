...
%It turns out there is a corresponding result for \emph{curtate}  (from the Latin \emph{curtus}, cut short) cycloids, curves traced by points inside a circle rolling on a line. When $b<a$, the curtate cycloid has inflection points, and the concave down arclength of one arch exceeds the concave up length by $8b$ (Figure ??). Each of these arclength theorems follows quickly from the other. But it took us a long time to realize this, so we save this result until the end (Section ??).
...
%Figure ?? shows an example closely related to the one in Figure ???. Here a circle (center $C_1}$ with radius $a$ rolls around the \emph{inside} of a circle with radius $2a$. The point $P^*$, $b$ units from the center of the rolling circle, traces an ellipse with major and minor semi-axes $b+a$ and $b-a$. The point $P$ attached to the rolling circle traces a diameter of ${\cal C}_2$ twice as ${\cal C}_1$ rolls once around ${\cal C}_2$. Our theorem for prolate hypocycloids then implies the same-side arclength (red) of the ellipse exceeds its opposite-side length (blue) by $4a$, twice the diameter of ${\cal C}_2$.
...
% Now the $(a,b,-2a)$ prolate hypocycloid is an ellipse with major and minor semi-axes $b+a$ and $b-a$. And its sister $(a,a,2a)$ hypocycloid is a twiced-traced diameter of the larger circle. So the same-side arclength of the ellipse exceeds its opposite-side length by $4a$, twice the diameter of the fixed circle. 
...
\section{The Trammel of Archimedes}
...
The other half is a second trammel (Figure ??). It slides along the same axes as the the one in Figure ?? and traces the same ellipse. Its tracing point $P^*$ lies between the pins, at distances $P^* R_2 = P^* R_1 = b+a$ and $P^* P_2 = P^* P_1 = b-a$ from them. So $\angle R_1 \cong\angle R_2$ and $\angle P_1\cong \angle P_2$ and in the animated figure the two arms rotate at the same rate in opposite senses as $P^*$ traces the ellipse. But unlike the first trammel with a sliding arm that is never tangent to the ellipse, the arm of the second is tangent to the ellipse exactly four times. And these four points of tangency are the CROSSING points separating the same and opposite-side arclengths of the ellipse.
