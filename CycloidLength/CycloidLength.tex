...
The other half is a second trammel (Figure ??). It slides along the same axes as the the one in Figure ?? and traces the same ellipse. It also has the same tracing point. But now $P^*$ lies \emph{between} the pins, at distances $P^* R_2 = P^* R_1 = b+a$ and $P^* P_2 = P^* P_1 = b-a$ from them. So the two arms cut the axes at the same angle (ie. $\angle R_1 \cong\angle R_2$ and $\angle P_1\cong \angle P_2$) and the arms rotate at the same rate in opposite senses as $P^*$ traces the ellipse. But while the arm of the first trammel is never tangent to the ellipse, the arm of the second is tangent to the ellipse exactly four times. And these four points of tangency are the CROSSING points separating the same and opposite-side arclengths of the ellipse.
...
You might also notice $P^*$ and the instantaneous centers of rotation $Q_1$ and $Q_2$ of the rolling circles are always colinear. This is because the trammel arms $\overline{P_1R_1}$ and ${\cal P_2R_2}$ make the same angles with the axes and so ${\cal C}_1$ and ${\cal C}_2$ turn at the same rate.
