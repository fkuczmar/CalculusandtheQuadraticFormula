...
Our proof of this theorem is primarily geometric and relies on the instantaneous center of rotation.
...
Figure ?? shows an example with a single transition point, crossed twice.  Here a circle rolls once around another of the same radius. The $(a,a,a)$ epicycloid is a cardioid and the $(a,b,a)$ prolate epicycloid a limacon with polar equation $r=2a+2b\cos\theta$ (the the pole is at the node and the polar axis points upward in the figure). The transition point is the node and we recover Robinson's result. The limacon's outer arclength exceeds the length of its loop by the length $2(8a) = 16a$ of the sister cardioid equal to twice that of the ordinary $a$-cycloid).
...
To understand this result geometrically, we need to identify the transition points and Robinson does just that. He shows 
the transition points divide the segments tangent to the ellipse (with their endpoints on its extended axes) into segments $P^*B = b+a$ and $P^*A = b-a$, with lengths respectively equal to the semi-major and semi-minor axes (see Figure ???). To see why, we turn to a more practical way of drawing an ellipse.
...
%In an episode of \emph{This Old House} Tom Silva uses the trammel of Archimedes to construct an ellipse. Trammel comes from \emph{trēmaculum}, a concatenation of the Latin \emph{trēs} (``three") and \emph{macula} (``mesh"). In middle English it referred to a three-layered fishing net and it has evolved to mean any restraint or hindrance (CITE A SOURCE HERE). And restraining two points $P$ and $Q$ on a thin rod to slide along two (usually perpendicular) is exactly what we need to force any third point $P^*$ on that rod (or more generally any point fixed in the reference frame of the rod) to trace an ellipse. %that we can draw an ellipse by tracing a third point $P^*$ on the rod. %(more generally, as the endpoints of a segment slide along two perpendicular lines, a point fixed in the reference frame of the segment sweeps out an ellipse). 

In an episode of \emph{Ask This Old House} Tom Silva uses the trammel of Archimedes to construct an ellipse. A trammel is a restraint or hindrance, and restraining two points on a thin rod to slide along two (usually perpendicular) lines is exactly what we need to force any third point on that rod (or any point fixed in the reference frame of the rod) to trace an ellipse.
