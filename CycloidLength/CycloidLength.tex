...
What if $b<a$? Then the tracing point lies inside the rolling circle and the \emph{curtate} (from the Latin \emph{curtus}, cut short) cycloid does not cross the $x$-axis. But it does have inflection points. And the convave-down arclength of one arch exceeds the concave-up length by $8b$ (Figure ??). Each of these arclength theorems follows quickly from the other. But it took us a long time to realize this, let alone to stumble upon the latter. So we save this discussion for the end (Section ??).
...
To better understand this result geometrically, we need to identify the transition points and Robinson does just that. The tangent segments at these points with their endpoints on the axes of the ellipse have length $2b$. Furthermore the points of tangency divide each segment into two segments with lengths $P^*B = b+a$ and $P^*A = b-a$, respectively equal to the semi-major and semi-minor axes (see Figure ? below).
...
To better understand why, we turn to a more practical way of drawing an ellipse.
...
The trammel of Archimedes hides in Figure ??, repeated here below, where the endpoints of the diameter $\overline{PR}$ of ${\cal C}_1$ through $P^*$ move along perpendicular lines. 
