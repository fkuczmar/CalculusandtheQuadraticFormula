...
the transition points divide the segments tangent to the ellipse, those with their endpoints on the extended axes, into segments $P^*B = b+a$ and $P^*A = b-a$, with lengths respectively equal to the semi-major and semi-minor axes (see Figure ???). To see why, we turn to a more practical way of drawing an ellipse.



%The tangent segments at these points with their endpoints on the axes of the ellipse have length $2b$. Furthermore the points of tangency divide each segment into two segments with lengths $P^*B = b+a$ and $P^*A = b-a$, respectively equal to the semi-major and semi-minor axes (see Figure ? below).
...
%To better understand why and to give an equivalent description of the transition points (as the points on the ellipse that minimize the length of its tangent segments running between the coordinate axes), we turn to a more practical way to draw an ellipse. %the trammel of Archimedes.


%The transition points have an equivalent description. They are the points of the ellipse that minimize the length of its tangent segments running between the coordinate axes. We turn to this next. %To understand why is the topic of the next section.
...
The other half is a second trammel (Figure ??). It has the same axes as the the one in Figure ?? and it traces the same ellipse. But its tracing point lies between the pins instead of outside. And unlike the first trammel whose sliding arm is never tangent to the ellipse, the arm of the second is tangent to the ellipse exactly four times. And these four points of tangency are the transition points separating the same and opposite-side arclengths.
...
This shows $P^*$ sweeps out an ellipse. To find the arm's position when it is tangent to the ellipse, imagine the arm in motion. The arm is then tangent to the ellipse exactly when $P^*$ is moving parallel to the arm, or when $dy/dx = -\tan\phi$. Some calculus and algebra then shows
\[
   (x,y) = (2b\cos^3 \phi, 2b\sin\phi).
\]

But we can avoid these computations by appealing once again to the center of curvature. Imagine the 
