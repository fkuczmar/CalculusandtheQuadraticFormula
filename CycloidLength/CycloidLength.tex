...
So at the transition points, when $\overline{Q_1P^*}$ is perpendicular to $\overline{C_1Q_1}$, (a) and (b) imply $\overline{Q_2P^*}$ is perpendicular to $\overline{P_2R_2}$ and hence (from (c)) $\overline{P_2R_2}$ is tangent to the ellipse. 

The proof of (a) and (b) rests on the fact that the radius of each rolling circle is half the radius of its partner. For then  
...
The key idea is to find the instantaneous center of rotation of the sliding arm. This is the point in the reference frame of the sliding arm momentarily at rest relative to the $xy$-plane in Figure ??. To find this center of rotation physically, place two adjacent vertices $P_2$, $R_2$ of a rectangular paper on adjacent edges of a tabe top (Figure ??) (segment $\overline{P_2R_2}$ is the trammel's arm, the adjacent edges the trammel's axes.) Now rotate the paper a bit about a random point $W$ and points $P_2$ and $R_2$ most likely head off the table. But choose the correct point (the arm's instantaneneous center of rotation) and the trammel's endpoints head along circular paths along (ie. tangent) to the adjacent edges. 
