...
To better understand this result geometrically, we need to identify the transition points and Robinson does just that. He shows 
the transition points divide the segments tangent to the ellipse, those with their endpoints on the extended axes, into segments $P^*B = b+a$ and $P^*A = b-a$, with lengths respectively equal to the semi-major $OX$ and semi-minor axes $OY$ (see Figure ???). To see why, we turn to a more practical way of drawing an ellipse.
...
Figure ?? below illustrates the reason why.
...
\href{https://www.desmos.com/calculator/2gnd3kgvyz}{Double Generation Ellipse 5}
